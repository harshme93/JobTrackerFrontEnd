...
function App() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "settings", "titles"), (snap) => {
      if (snap.exists()) setTitles(snap.data().list || []);
    });
    return unsub;
  }, []);
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "settings", "last_run"), (snap) => {
      if (snap.exists()) setLastRun(snap.data());
    });
    return unsub;
  }, []);
  ...
  const addCompany = async () => {
    if (!newCompany.name || !newCompany.url) { flash("Fill all fields"); return; }
    const id = newCompany.name.toLowerCase().replace(/\s+/g, "-") + "-" + Date.now();
    await setDoc(doc(db, "companies", id), { name: newCompany.name.trim(), url: newCompany.url.trim() });
    setNewCompany({ name: "", url: "" });
    flash("Company added");
  };

  const deleteCompany = async (id) => { await deleteDoc(doc(db, "companies", id)); flash("Deleted"); };

  const startEdit = (c) => { setEditingId(c.id); setEditData({ name: c.name, url: c.url }); };

  const saveEdit = async (id) => {
    await setDoc(doc(db, "companies", id), { name: editData.name.trim(), url: editData.url.trim() });
    setEditingId(null);
    flash("Updated");
  };
  ...
  const addTitle = async () => {
    ...
    const newTitles = newTitle.split(",").map((t) => t.trim().toLowerCase()).filter(Boolean);
    const updated = [...new Set([...titles, ...newTitles])];
    await setDoc(doc(db, "settings", "titles"), { list: updated });
    setNewTitle("");
    flash("Title(s) added");
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        <h2 style={styles.h2}>Job Titles to Track</h2>
        <p style={styles.hint}>These titles are checked against ALL companies.</p>
        <div style={styles.inputRow}>
          <input
            style={{ ...styles.input, marginBottom: 0, flex: 1 }}
            placeholder="e.g. senior analyst, product manager"
            value={newTitle}
            onChange={(e) => setNewTitle(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && addTitle()}
          />
          <button style={styles.btn} onClick={addTitle}>Add</button>
        </div>
        <div style={styles.titles}>
          {titles.map((t, i) => (
            <span key={i} style={styles.tag}>
              {t}<span style={styles.tagX} onClick={() => removeTitle(t)}>×</span>
            </span>
          ))}
        </div>
        {titles.length === 0 && <p style={styles.empty}>No titles added yet.</p>}
      </div>

      <div style={styles.card}>
        <h2 style={styles.h2}>Add Company</h2>
        <input style={styles.input} placeholder="Company name" value={newCompany.name} onChange={(e) => setNewCompany({ ...newCompany, name: e.target.value })} />
        <input style={styles.input} placeholder="Careers page URL" value={newCompany.url} onChange={(e) => setNewCompany({ ...newCompany, url: e.target.value })} />
        <button style={styles.btn} onClick={addCompany}>Add Company</button>
      </div>
          ...
          <div key={c.id} style={styles.companyRow}>
            {editingId === c.id ? (
              <div style={styles.editBlock}>
                <input style={styles.input} value={editData.name} onChange={(e) => setEditData({ ...editData, name: e.target.value })} />
                <input style={styles.input} value={editData.url} onChange={(e) => setEditData({ ...editData, url: e.target.value })} />
      ...
      <div style={styles.card}>
        ...
        <button style={styles.btn} onClick={saveEmail}>Save Email</button>
...
const styles = {
  ...
  hint: { fontSize: "0.8rem", color: "#888", marginBottom: "0.5rem", marginTop: "-0.5rem" },
  ...
  btn: { background: "#111", color: "#fff", border: "none", borderRadius: 6, padding: "0.6rem 1.25rem", cursor: "pointer", fontSize: "0.9rem", fontWeight: 500, whiteSpace: "nowrap" },
  ...
  titles: { display: "flex", flexWrap: "wrap", gap: "0.4rem" },
  tag: { background: "#e0e0e0", borderRadius: 4, padding: "0.2rem 0.6rem", fontSize: "0.8rem", display: "flex", alignItems: "center", gap: "0.4rem" },
  tagX: { cursor: "pointer", fontWeight: 700, fontSize: "1rem", lineHeight: 1, color: "#999" },
  editBlock: { width: "100%" },
